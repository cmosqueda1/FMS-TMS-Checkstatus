...
// Helper: normalize values for strict-but-trimmed matching
...
export default async function handler(req, res) {
  ...
  // Unique, trimmed PUs, cap at 150 (same cap as PRO route)
  const MAX_PUS = 150;
  const uniquePus = [...new Set(pu_nos.map(p => cleanStr(p)).filter(Boolean))];
  const trimmedPus = uniquePus.slice(0, MAX_PUS);

  try {
    const results = await checkAllByPu(trimmedPus);
...
======================== */
async function checkAllByPu(pus) {
  if (!FMS_USER || !FMS_PASS) {
    throw new Error("Missing FMS_USER / FMS_PASS in environment");
  }

  // 1) FMS: auth + search by PU + DO mapping
  const fmsToken     = await authFms();
  const searchJsonPu = await fmsSearchOrdersByPu(fmsToken, pus);
  const fmsMap       = buildFmsMapByPu(searchJsonPu);
  ...
  const results = await runWithConcurrency(pus, CONCURRENCY, async (pu) => {
    ...
    if (tmsAuth && tmsMap) {
      ...
      try {
        const row = tmsMap.get(cleanStr(pu));
        if (!row) {
...
async function fmsSearchOrdersByPu(token, pu_nos) {
  ...
  const body = {
    ...
    status: [], sub_status: [],
    tracking_nos: [],               // 🔹 empty so search is PU-based
    trips: []
  };
...
function buildFmsMapByPu(searchJson) {
  ...
  for (const it of items) {
    // TODO: adjust these property names once you confirm FMS response shape
 ...
 * using the same structure but input_filter_pu instead of input_filter_pro.
 */
async function tmsTraceForPus(auth, puList) {
  ...
  body.set("input_filter_pro", ""); // 🔹 clear PRO filter
  body.set("input_filter_trip", "");
  body.set("input_filter_order", "");
  body.set("input_filter_pu", puList.map(cleanStr).join("\n")); // 🔹 PU filter
  ...
  for (const rw of rows) {
    // TODO: Adjust these property names to whatever TMS uses for Pickup # in trace
    const key = cleanStr(
      rw.tms_order_pu ??
      rw.pu_no ??
      rw.pu ??
